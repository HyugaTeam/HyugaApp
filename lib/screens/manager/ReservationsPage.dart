...
class _ReservationsPageState extends State<ReservationsPage> {
  ...
  // Future<void> fetchPendingReservations() async{
  //   FirebaseFirestore _db = FirebaseFirestore.instance;
  //   pendingReservations =  (await _db.collection('users').doc(authService.currentUser.uid).collection('managed_locals')
                                ...
                                child: Column(
                                  children: [
                                    Container(
                                      ...
                                      child: Text("Nume: "+ pendingReservations[index].data()['guest_name'], style: TextStyle(fontSize: 22),)
                                    ),
                                    Container(
                                      padding: EdgeInsets.symmetric(vertical: 20),
                                      child: Text(
                                        "Pentru ora: "+ DateFormat("Hm").format(DateTime.fromMicrosecondsSinceEpoch(
                                        pendingReservations[index].data()['date_start'].millisecondsSinceEpoch
                                        )).toString()
                                        ,
                                    ...
                                    Container(
                                      padding: EdgeInsets.symmetric(vertical: 20),
                                      child: Text(
                                        "Nr. persoane: " + "${pendingReservations[index].data()['number_of_guests']}",
                                        style: TextStyle(fontSize: 18),
                                      ),
                                    ),
                                    Container(
